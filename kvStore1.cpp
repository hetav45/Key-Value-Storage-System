...
private:
    ...
    NodePtr searchTreeHelperDebug(NodePtr node, char *key)
    {
        cout << node->data << endl;
        cout << strcmp(key, node->data) << endl << endl;
    ...
    void printHelper(NodePtr root, string indent, bool last)
    {
        if (root != TNULL)
        {
            ...
            cout << root->data << " "<< root->value << " " << "(" << sColor << ")  " << root->leftNo << "   " << root->rightNo << endl;
...
public:
    ...
    bool getDebug(Slice &key, Slice &value)
    {
        char *key1 = (char *)malloc((key.size+1)*(sizeof(char)));
        strncpy(key1,key.data,key.size);
        key1[key.size]=0;
        ...
        NodePtr temp = searchTreeHelperDebug(this->root[index1][index2], key1);
        if (temp == TNULL)
        {
            pthread_mutex_unlock(&mutex[index1][index2]);
